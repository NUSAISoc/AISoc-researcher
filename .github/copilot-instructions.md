...
- Before any research-content edit, read `.beryl/agent/synchronization-contract.md` and treat every target it names as mandatory. Do not finish until shared facts are synchronized and report Markdown, LaTeX, Word, PDF, and hash artifacts are refreshed where applicable.
- Before problem identification, solution design, or proposing an improvement, read `.beryl/agent/systems-thinking-methodology.md`. Frame the work through system boundary, actors, feedback, delays, assumptions, and a leverage hypothesis; use a conceptual CLD or an evidence-appropriate SFD.
...
- `academic-research-writer`: writing/editing literature review, problem analysis, research question, solution design, experiment design, statistical analysis, evaluation, proposal, report, notes, references.
- `voice-and-confidence-calibration`: voice pass for the user-facing surfaces (`ProjectProposal.md`, `report/`, `notes/`).
- `clearly-and-concisely-academic`: scholarly clarity pass (calibrated hedging, active voice, AI-tell removal) for any academic prose.
- `planning`: plans, designs, approaches, and feature planning gates.
- `initial-build`: clarifies, plans, ratifies, and implements the solution application (only after the PRD is green-lit; built as a submodule under `solution/`).
- `adding-features`: feature implementation after a user-ratified plan, including experiment-harness work under the Autoresearch Sandbox Gate.
...
- `grill-me`: non-trivial research-design decision, cross-cutting change, ambiguous bug fix.
...
- `frontend-design`: distinctive, intentional visual design for the solution UI (after green-light only).
